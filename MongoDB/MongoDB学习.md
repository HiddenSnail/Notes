...
副本集为APP提供了冗余和高可用的特性，并且在一定条件下可以增加读取容量，因为客户端可以把读请求发送到不同的服务器上。
...
#### 3.14.13 Journal
Journal是WiredTiger引擎的预写日志，是MongoDB可以在故障时仍能提供持久性的基本方式。
...
```
    ...
    # 把Journal中该时间点之后的操作全部重放一遍
    3. Apply the operations in the journal files since the last checkpoint.
```

WiredTiger保存Journal文件到磁盘中，只会出现在以下条件中：
+ 对于副本集成员来说(包括主节点和副本节点)
  + 出现有需要查询oplog的操作的
    + 向前扫描oplog的查询
    + 需要保证因果一致性的读操作
  + 对于副本节点来说，在批量应用完oplog之后 

+ 写请求中出现j=true选项，或隐含该层含义的选项，如w=majority
+ 每过100ms
+ 当WiredTiger创建新的Journal文件时

单个Journal文件的大小约为100MB，当写入的内容超过100MB时，会新建另一个Journal文件。
WiredTiger会自动删除lastCheckpoint之前的文件。
...
#### 4.1.1.1 分片键(Shard Key)

##### 分片键的作用
分片键的本质其实是索引，它比普通索引多了一个作用，就是它还可以做到对chunk的索引，进而mongos可以做到把请求发送到指定分片上。
在分片集群种，假如查询条件包含了分片键，则mongos只会把请求发送到保存对应数据的分片上进行查询；否则，将会把请求都广播给所有分片。
因此，在分片集群中，使用分片键查询要比使用普通索引查询效率更高 


##### 分片键的选择
###### 选取目标
1. 数据能均匀地分布在MongoDB的分片中
2. 分片键能够覆盖最常用的查询模式

###### 选取原则
1. 选择基数高字段作为分片键（基数：指由字段值的组成的set中元素的个数，此处请注意set的概念：由一组无序且唯一的项组成数据集）
   + 分片键的基数将决定chunk个数的上限，而chunk的个数上限则决定shard的上限。若基数太小，则会影响集群的水平扩展有效性(水平扩展即指添加shard的方式提升MongoDB性能，对应的垂直扩展指通过增加硬件能力的方式提升MongoDB性能)，影响方式有以下2种：
     + 随着chunk数据量的增大至chunksize时，chunk会进行拆分。由于基数太小，每个chunk会快速变为不可拆分的chunk。当chunk不可再拆分时，chunk的数据量会持续增长(即使超过了chunksize)，形成巨型chunk，进行影响读写性能。
     + 当chunk都演变为不可拆分chunk时，chunk数目不会再增加，其数目也就是分片键的基数。而一个shard至少要承载一个chunk，也就是说shard的数目至多也就是分片键的基数。超出基数的shard，由于没有承载chunk，因此无法给集群性能带来提升。因此，分片键基数会影响分片集群的扩展能力。
   + 如果待分片的片键基数确实很小，可以考虑使用复合索引字段，来增加达到增加基数的目的
2. 选择数据频率均匀的字段作为分片键
   + 若分片键的某些值出现的频度很高，那么存储高频度分片键值对应文档的chunk将会一再分割，直至不可再分(即一个chunk占用一个分片)，一旦chunk不可再分，则chunk数据会持续增长，形成举行chunk，表现为集群中的某几个shard的数据量很高但却无法进行chunk迁移，最后将会影响集群的水平扩展的有效性。
   + 若确实需要使用高频度值的字段作为分片键，可以考虑配合使用一些低频度或唯一值的字段组合成复合索引，然后使用复合索引字段来做分片键。
3. 选择不具有单调变化性质的字段作为分片键
   + 当集合分片后，会生成2个边界chunk，即上边界为maxKey的chunk和下边界为minKey的chunk。分片键字段单调递增时，在数据总会插入到上边界为maxKey的chunk中；当分片键字段单调递减时，数据总会插入到下边界为minKey的chunk中。这将会影响写入效率。
   + 若确实需要使用具有单调变化性质的字段作为分片键，则可以考虑使用==哈希分片==机制
   + 当使用_id进行分片时，请使用哈希分片机制，因为MongoDB里的_id时默认单调递增的

##### 哈希分片机制 VS 范围分片机制
哈希分片：
+ 优点：
  + 可以使数据更均匀地分布在各个chunk上，并且可以避免单调变化的分片键值带来的写入瓶颈
+ 缺点: 
  + 对于范围查询不友好，mongos会以广播的方式进行范围查询。
  + 哈希索引会将浮点数截断为64位整数，再进行哈希值的计算，因此当分片键值为浮点数时，为了避免冲突，最好不要采用哈希索引。
+ 适用范围：
  + 理想的哈希分片键的选择应保证3点：片键基数大、片键值出现频率均匀、片键值具有单调性

4.4版本开始，MongoDB支持复合hash分片机制，即分片键的前缀字段可以为范围索引字段，而后面的字段为hash索引字段。
```
sh.shardCollection("testsk.hashtb",{"key":1, "value":"hashed"})
```

范围分片：
+ 优点：
  + 由于是基于连续的范围进行chunk拆分，对于范围性的查询是比较友好的
+ 缺点：
  + 可能会因为分片键选择不佳，导致读写效率降低
+ 适用范围：
  + 理想的范围分片键的选择应保证3点：片键基数大、片键值出现频率均匀、片键值不具有单调性



##### 分片键注意事项

- 4.4版本及之后，插入的集合的document可以不包含分片键字段，而4.2版本及之前，则必须包含分片键字段。

- 4.2版本之前，片键的值是不可以修改的，4.2版本及之后，片键的值是可以修改的，除非分片键为_id
  ```
  # 假如存在集合coll，该集合的字段如下
  coll {
          _id,
  ...
  # 若该集合以name作为分片片键时，则4.2之前的版本，coll集合中的document是不可以使  用update方法修改name字段的，4.2及之后的版本则可以修改
  ```

- 关于片键的重新选取问题
  > // 从5.0版本开始，你可以通过修改分片片键，重新对集合进行分片
  > Starting in MongoDB 5.0, you can reshard a collection by changing a   document's shard key.
  > // 从4.4版本开始，你可以通过在现有片键上添加后缀字段，来优化分片
...
- 对非空集合进行分片，那么该集合必须存在片键字段本身的索引，或者以片键字段为==前缀==的复合索引；对于空集合进行分片，`sh.shardCollection()`会自动使用分片键创建索引。
  ```
  # 假如存在集合coll，该集合的字段如下
  coll {
          _id,
  ...
  # 当该集合非空时，假如需要以age作为分片片键时，则需要满足一下条件之一
  1. coll集合已经建立了age字段的索引
  2. coll集合已经建立age字段为 [ 前缀 ] 的复合索引，如age:1 + name:1，注意必须是  前缀，否则集合将无法进行分片  
  ```

- 一旦对集合进行分片，它的片键域不能更改，并且片键域的值也不能更新
  ```
  # 假如插入document
  {
        "_id":"2222",
  ...
  # 以a作为片键域，则一旦该document插入集合中，则该document的a的值不能更改
  ```

- 当集合中存在UniqueIndex时，只能以该UniqueIndex或者该UniqueIndex的前缀作为分片键，否则集合分片会失败。PS：唯一索引(UniqueIndex)，用于保证字段值的唯一性，如发现插入唯一索引的字段的值重复的文档，插入将会失败。

#### 4.1.1.2 shard
- 对于每个数据库，其没有分片的集合都会保存在一个称为主分片(primary shard)的分片中。这个主分片是由mongos通过判断每个分片的数据大小决定的，分片的占用数据量越小越可能成为主分片。

- 使用`db.adminCommand( { movePrimary : "test", to : "shard01" } )`命令可以将test库的主分片迁移为shard01。在执行该命令的过程中，禁止读写该库的任何未分片集合，直到迁移完成。

#### 4.1.1. config servers
- config server会保存整个分片集群的元数据，这些元数据描述了数据和分片的状态和组织方式。mongos会缓存这些元数据，用于将请求路由到正确的分片上。

- 一组config servers只能用于一个分片集群，不要多个集群使用同一组config servers

- 对config server进行维护操作，会对整个分片集群的性能和可用性产生重大影响，集群可能在一段时间内处于只读或离线状态。

- 配置config server副本集，必须满足：
  + 没有仲裁节点
  + 没有delayed成员
  + 必须构建索引

- config server主节点挂掉后，假如无法选举出新的主节点，你仍可以往集群里读写数据，只是不会进行chunk迁移和chunk拆分？直到主节点选举出来。当所有节点不能用时，整个集群会进入无法操作的状态

- ==在对config server操作之前==，总是要备份Config数据库

#### 4.1.1.3 mongos
- mongos可以部署在打算使用数据库的应用服务器上，可以减小mongos到app直接的网络延迟。当然也可以部署在mongod服务器上，或者专门使用一台服务器。

- mongos在进行数据查询/插入时，会采用 Targeted Operations 和 Broadcast Operations 两种模式。Targeted模式效率会更高，使用Targeted模式的一般需要在操作时指定分片键字段，因此好的分片键应该尽量覆盖常用的查询模式。
...
```
profile <级别>： 0（关闭）1（仅统计时间超过slowms的操作）2（记录所有操作）
slowms <整数>：表示slowOp的时间阈值
slowOpSampleRate <[0-1]之间的数值>：表示采样比率，如配置为0.4，则记录40%的slowOp

如果是通过shell开启，则下次启动时会关闭；如果使用配置文件配置，则可以启动时开启。
